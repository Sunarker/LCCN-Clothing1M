...
tf.app.flags.DEFINE_string('init_dir', 'events_ce/clothing1M_train',
                           ...
                           """and checkpoint.""")
tf.app.flags.DEFINE_string('train_dir', 'events_varT/clothing1M_train',
                           """Directory where to write event logs """
                           """and checkpoint.""")
...
def train(T):
  ...
  with tf.Graph().as_default():
    ...
    # adaption layer
    logits_T = tf.get_variable('logits_T',shape=[clothing1M.NUM_CLASSES+1,clothing1M.NUM_CLASSES],initializer=tf.constant_initializer(np.log(T + 1e-8)))
    adaption_layer = tf.nn.softmax(logits_T)

    # label adaption 
    preds_aug = tf.clip_by_value(tf.matmul(preds,adaption_layer), 1e-8, 1.0 - 1e-8)
    logits_aug = tf.log(preds_aug)

    # Calculate loss.
    loss = clothing1M.loss(logits_aug, labels)
