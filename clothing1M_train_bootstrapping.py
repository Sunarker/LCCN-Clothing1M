...
tf.app.flags.DEFINE_string('init_dir', 'pretrained/resnet_own',
                           ...
                           """and checkpoint.""")
tf.app.flags.DEFINE_string('train_dir', 'events_bootstrapping/clothing1M_train',
                           """Directory where to write event logs """
                           """and checkpoint.""")
  ...
  with tf.Graph().as_default():
    ...
    class _LoggerHook(tf.train.SessionRunHook):
      ...
      def before_run(self, run_context):
        self._step += 1
        return tf.train.SessionRunArgs(_LoggerHook_loss)  # Asks for loss value.
    ...
    with tf.train.MonitoredTrainingSession(
      ...
      while not mon_sess.should_stop():
        res = mon_sess.run([train_op,acc_op,global_step],feed_dict={is_training:True, alpha: 0.5})
